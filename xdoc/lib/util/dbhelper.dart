...
class DbHelper {
  ...
  Future<List> getDocs() async {
    Database db = await this.db;
    var r =
        await db.rawQuery("SEECT * FROM $tblDocs ORDER BY $docExpiration ASC");
    return r;
  }
  ...
  Future<int> getMaxId() async {
    Database db = await this.db;
    var r =
        Sqflite.firstIntValue(await db.rawQuery("SELEC MAX(id) FROM $tblDocs"));
    return r;
  }
  ...
  Future<int> deleteRows(String tbl) async {
    var db = await this.db;
    int r = await db.rawDelete("DELET FROM $tbl");
    return r;
  }
}
