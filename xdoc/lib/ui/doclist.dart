...
class DocListState extends State<DocList> {
  ...
  Future getData() async {
    ...
    dbFuture.then(
      ...
      docsFuture.then(
          // result here is the list docs in the database.
          (result) {
        if (result.length >= 0) {
          List<Doc> docList = List.empty();
        ...
        itemBuilder: (BuildContext context, int position) {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                (Val.GetExpiryStr(this.docs[position].expiration) +
                    dl +
                    '\nExp: ' +
                    DateUtil.convertToDateFull(this.docs[position].expiration)),
              ),
              onTap: () {
      ...
      body: Center( 
        child: Scaffold(  
          ...
          floatingActionButton: FloatingActionButton( 
            onPressed: () {
              navigateToDetail(Doc.withId(-1, '', '', 1, 1, 1));
            },
            tooltip: 'Add new doc',
